...
from app.storage import (
    init_db, save_session, load_user_sessions, load_session, delete_session,
...
def _get_user_session(session_id: str, user_id: str) -> ExamSession:
    """Load a session belonging to this user, or raise 404."""
...
# =====================
# Form Generation (no auth needed)
# =====================
...
def create_session(req: AnswerKeyRequest, user_id: str = Depends(get_current_user)):
...
def delete_session_endpoint(session_id: str, user_id: str = Depends(get_current_user)):
    ...
    _get_user_session(session_id, user_id)  # verify ownership
    delete_session(session_id, user_id)
    return {"message": "Session deleted"}


@app.delete("/api/sessions/{session_id}/results/{result_index}")
def delete_result_endpoint(session_id: str, result_index: int, user_id: str = Depends(get_current_user)):
...
def upload_roster(session_id: str, req: RosterUploadRequest, user_id: str = Depends(get_current_user)):
...
def get_roster(session_id: str, user_id: str = Depends(get_current_user)):
...
async def upload_roster_pdf(session_id: str, file: UploadFile = File(...), user_id: str = Depends(get_current_user)):
...
def _process_scan(image: np.ndarray, answer_key: dict = None,
                  session: ExamSession = None, num_questions: int = 40,
                  answer_key_b: dict = None, use_booklet: bool = False,
                  num_options: int = 5) -> ScanResponse:
    """Core scan processing pipeline."""
    try:
        return _process_scan_inner(image, answer_key, session,
                                    num_questions, answer_key_b, use_booklet, num_options)
    except Exception as e:
        import traceback
        traceback.print_exc()
        return ScanResponse(success=False, error=f"Sunucu hatasi: {str(e)}")
...
def _process_scan_inner(image: np.ndarray, answer_key: dict = None,
    ...
    if session:
        has_roster = bool(session.roster and session.roster.students)
        logger.info(f"OCR: session found, has_roster={has_roster}, "
                     f"student_count={len(session.roster.students) if has_roster else 0}")
        if has_roster:
            ocr.set_roster([
                {"name": s.name, "surname": s.surname,
                 "student_number": s.student_number}
                for s in session.roster.students
            ])
    ...
    if warped is not None:
        ...
        roster_surname = ""
        matched_number = ""
        if session and session.roster and session.roster.students and number_field.text:
            ...
            # 3. Fuzzy match — allow only 1 wrong digit
            if not roster_name:
                best_match = None
                best_diff = 99
                for s in session.roster.students:
                ...
                if best_match and best_diff == 1:
                    same_diff_count = sum(
                        1 for s in session.roster.students
...
async def scan_sheet(
    ...
    if session_id:
        session = _get_user_session(session_id, user_id)
        key = session.answer_key
        key_b = session.answer_key_b
        use_bk = session.use_booklet
        num_questions = session.num_questions
        num_opts = session.num_options
    elif answer_key:
        try:
    ...
    response = _process_scan(img, key, session, num_questions, key_b, use_bk, num_opts)
...
def get_pending_reviews(session_id: str, user_id: str = Depends(get_current_user)):
...
def verify_result(session_id: str, req: VerificationRequest, user_id: str = Depends(get_current_user)):
...
def get_stats(session_id: str, user_id: str = Depends(get_current_user)):
    session = _get_user_session(session_id, user_id)
...
def export_results(session_id: str, user_id: str = Depends(get_current_user)):
